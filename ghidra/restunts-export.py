...
# By default this script expect your Ghidra project to be stored in the
# /ghidra folder in the restunts repo. If that doesn't suit you, set the path to
# the root of the restunts project dir here. The files produced by this script
# are placed relative to this directory. All required subfolders must be
# created in advance.
...
banner = "; Generated by {script} (Ghidra)\n".format(script=os.path.basename(inspect.getsourcefile(lambda:0)))
...
# Data directory of the program being analysed.
prog_dir = state.getProject().getProjectLocator().getProjectDir().getAbsolutePath()
# restunts ghidra/ folder.
proj_dir = PROJECT_DIR_OVERRIDE if PROJECT_DIR_OVERRIDE else os.path.normpath(os.path.join(prog_dir, "../.."))
# restunts src/ folder.
